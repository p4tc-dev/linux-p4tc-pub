...
int fillup_metact_cmds(struct sk_buff *skb, struct list_head *meta_operations)
{
	...
	list_for_each_entry(entry, meta_operations, meta_operations) {
		...
		if (entry->opA) {
			nest_opnd = nla_nest_start(skb, TCAA_METACT_OPER_A);
		...
		if (entry->opB) {
			nest_opnd = nla_nest_start(skb, TCAA_METACT_OPER_B);
		...
		if (entry->opC) {
			nest_opnd = nla_nest_start(skb, TCAA_METACT_OPER_C);
...
static struct p4tc_type_mask_shift *
create_constant_bitops(struct tca_meta_operand *kopnd, struct p4tc_type *t,
		       struct netlink_ext_ack *extack)
{
	struct p4tc_type_mask_shift *mask_shift;
...
static const struct nla_policy metact_policy_oper[TCAA_METACT_OPND_MAX + 1] = {
	[TCAA_METACT_OPND_INFO] = { .type = NLA_BINARY,
				    .len = sizeof(struct tca_u_meta_operand) },
			       ...
			       struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested(tb, TCAA_METACT_OPND_MAX, nla,
			       metact_policy_oper, extack);
	if (err < 0) {
...
static const struct nla_policy metact_ops_policy[TCAA_METACT_OPER_MAX + 1] = {
	[TCAA_METACT_OPERATION] = { .type = NLA_BINARY,
				    .len = sizeof(struct tca_u_meta_operate) },
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested(tb, TCAA_METACT_OPER_MAX, nla, metact_ops_policy,
			       extack);
	if (err < 0) {
...
static const struct nla_policy metact_policy[TCA_METACT_MAX + 1] = {
	[TCA_METACT_PARMS] = { .type = NLA_BINARY,
			       .len = sizeof(struct tc_metact) },
		       ...
		       u32 flags, struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested(tb, P4TC_ACT_MAX, nla, NULL, NULL);
	if (err < 0) {
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested(oplist_attr, TCA_METACT_LIST_MAX, nla, NULL,
			       extack);
	if (err < 0)
		return err;
			   ...
			   struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested(tb, TCA_METACT_MAX, nla, metact_policy, extack);
	if (err < 0)
		return err;
		    ...
		    int ref)
{
	...
	nest = nla_nest_start(skb, TCA_METACT_LIST);
	...
	nest_parms = nla_nest_start(skb, TCA_METACT_ACT_PARMS);
	if (!nest_parms)
		goto nla_put_failure;
