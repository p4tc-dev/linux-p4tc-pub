...
static __net_init int tcf_act_base_init_net(struct net *net)
{
	struct tcf_dyn_act_net *base_net = net_generic(net, dyn_act_net_id);
	rwlock_t _act_mod_lock = __RW_LOCK_UNLOCKED(_act_mod_lock);

	idr_init(&base_net->act_base);
	base_net->act_mod_lock = _act_mod_lock;
	return 0;
}

static void __net_exit tcf_act_base_exit_net(struct net *net)
{
	struct tcf_dyn_act_net *base_net = net_generic(net, dyn_act_net_id);
	struct tc_action_ops *ops;
	unsigned long opid, tmp;

	idr_for_each_entry_ul(&base_net->act_base, ops, tmp, opid) {
		idr_remove(&base_net->act_base, ops->id);
	}

	idr_destroy(&base_net->act_base);
}

static struct pernet_operations tcf_act_base_net_ops = {
	.init = tcf_act_base_init_net,
	.exit = tcf_act_base_exit_net,
	.id = &dyn_act_net_id,
...
int tcf_register_dyn_action(struct net *net, struct tc_action_ops *act)
{
	...
	write_lock(&base_net->act_mod_lock);

	act->id = TCA_ID_DYN;

	ret = idr_alloc_u32(&base_net->act_base, act, &act->id,
			    TCA_ID_MAX, GFP_ATOMIC);
	if (ret < 0)
		goto err_out;
	write_unlock(&base_net->act_mod_lock);
...
int tcf_unregister_dyn_action(struct net *net, struct tc_action_ops *act)
{
	...
	write_lock(&base_net->act_mod_lock);
	if (!idr_remove(&base_net->act_base, act->id))
		err = -EINVAL;
...
static struct tc_action_ops *tc_lookup_action_n(struct net *net, char *kind)
{
	...
	if (kind) {
		...
		read_lock(&base_net->act_mod_lock);
		idr_for_each_entry_ul(&base_net->act_base, a, tmp, id) {
			if (strcmp(kind, a->kind) == 0) {
				if (try_module_get(a->owner))
					res = a;
				break;
...
static int __init tc_action_init(void)
{
	...
	register_pernet_subsys(&tcf_act_base_net_ops);
	return 0;
}
