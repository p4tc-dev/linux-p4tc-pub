...
static int p4tc_memcmp_le(const void *a, const void *b, size_t n)
{
        const u8 *a8 = a;
        const u8 *b8 = b;
        size_t i = n;
...
static int p4tc_memcmp(const void *a, const void *b, size_t n)
{
#if defined(__BIG_ENDIAN_BITFIELD)
	return memcmp(a, b, n);
#else
	return p4tc_memcmp_le(a, b, n);
#endif
}
	    ...
	    struct p4tc_type *src_t, void *srcv)
{
	...
	return p4tc_memcmp(a, b, sizeof(a));
}
...
static const struct p4tc_type_ops rate_ops = {
	.validate_p4t = p4t_u64_validate,
	.create_bitops = p4t_u64_bitops,
	.host_read = p4t_u64_hread,
	.host_write = p4t_u64_write,
...
int p4tc_register_types(void)
{
	...
	p4tc_register_type(P4TC_T_KEY, P4TC_MAX_KEYSZ, P4TC_MAX_KEYSZ, "key",
			   &key_ops);
	p4tc_register_type(P4TC_T_RATE, 64, 64, "rate", &rate_ops);
