...
static inline int p4tc_register_type(int typeid, size_t bitsz,
				    size_t container_bitsz,
				    const char *t_name,
				    struct p4tc_type_ops *ops)
{
	if (__p4tc_register_type(typeid, bitsz, container_bitsz, t_name, ops) < 0) {
...
{
	if (p4tc_register_type(P4T_U8, 8, 8, "u8", &u8_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_U16, 16, 16, "u16", &u16_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_U32, 32, 32, "u32", &u32_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_U64, 64, 64, "u64", &u64_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_U128, 128, 128, "u128", &u128_ops) < 0)
		return -1;

	if (p4tc_register_type(P4T_S8, 8, 8, "s8", &s8_ops) < 0)
		return -1;
	...
	if (p4tc_register_type(P4T_S16, 16, 16, "s16", &s16_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_S32, 32, 32, "s32", &s32_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_S64, 64, 64, "s64", &s64_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_S128, 128, 128, "s128", &s128_ops) < 0)
		return -1;

	if (p4tc_register_type(P4T_STRING, P4T_MAX_STR_SZ * 4,
			      P4T_MAX_STR_SZ * 4, "string",
			      &string_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_NUL_STRING, P4T_MAX_STR_SZ * 4,
			      P4T_MAX_STR_SZ * 4, "nullstr",
			      &nullstring_ops) < 0)
		return -1;

	if (p4tc_register_type(P4T_FLAG, 32, 32, "flag", &flag_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_PATH, 0, 0, "path", &path_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_MSECS, 0, 0, "msecs", &msecs_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_MACADDR, 48, 64, "mac", &mac_ops) < 0)
		return -1;
	if (p4tc_register_type(P4T_IPV4ADDR, 32, 32, "ipv4", &ipv4_ops) < 0)
		return -1;
