...
static struct p4tc_register *
tcf_register_create(struct net *net, struct nlmsghdr *n,
		    ...
		    struct netlink_ext_ack *extack)
{
	...
	if (reg_id) {
		reg->reg_id = reg_id;
		ret = idr_alloc_u32(&pipeline->p_reg_idr, reg,
				    &reg->reg_id, reg->reg_id,
				    GFP_KERNEL);
		if (ret < 0) {
			NL_SET_ERR_MSG(extack, "Unable to allocate register id");
			goto free_reg_value;
		}
	} else {
		reg->reg_id = 1;
...
static struct p4tc_register *
tcf_register_update(struct net *net, struct nlmsghdr *n,
		    struct nlattr *nla, u32 reg_id,
		    struct p4tc_pipeline *pipeline,
		    struct netlink_ext_ack *extack)
{
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (ret < 0) {
		if (i == 0) {
			NL_SET_ERR_MSG(extack,
				       "Unable to flush any register");
			goto out_nlmsg_trim;
		} else {
			NL_SET_ERR_MSG(extack,
				       "Unable to flush all registers");
		}
	}
...
static int tcf_register_dump(struct sk_buff *skb,
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	if (!ctx->ids[P4TC_PID_IDX]) {
		pipeline = tcf_pipeline_find_byany(*p_name, ids[P4TC_PID_IDX],
						   extack);
		if (IS_ERR(pipeline))
			return PTR_ERR(pipeline);
