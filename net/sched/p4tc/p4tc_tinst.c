...
static const struct nla_policy p4tc_tinst_policy[P4TC_TINST_MAX + 1] = {
	[P4TC_TINST_CLASS] = { .type = NLA_STRING, .len = TCLASSNAMSIZ },
	[P4TC_TINST_NAME] = { .type = NLA_STRING, .len = TINSTNAMSIZ },
	[P4TC_TINST_MAX_ENTRIES] =
		NLA_POLICY_RANGE(NLA_U32, 1, P4TC_MAX_TIENTRIES),
};
				...
				struct netlink_ext_ack *extack)
{
	...
	pipeline = tcf_pipeline_find_byid(tinst->common.p_id);
	     ...
	     char **p_name, u32 *ids, struct netlink_ext_ack *extack)
{
	...
	ret = nla_parse_nested(tb, P4TC_TINST_MAX, nla, p4tc_tinst_policy,
			       extack);
	if (ret < 0)
		return ERR_PTR(ret);
			...
			struct netlink_ext_ack *extack)
{
	...
	if (nla) {
		ret = nla_parse_nested(tb, P4TC_TINST_MAX, nla,
