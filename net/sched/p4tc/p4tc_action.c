			     ...
			     struct netlink_ext_ack *extack)
{
	...
	value = kzalloc(param->bytesz, GFP_KERNEL_ACCOUNT);
	if (!value) {
		kfree(nparam);
		return ERR_PTR(-ENOMEM);
	}
				    ...
				    struct tcf_p4act *p4act)
{
	...
	if (p4act_params) {
		idr_for_each_entry_ul(&p4act_params->params_idr, param, tmp,
				      param_id) {
			if (!p4a_param_has_runt_flag(param))
				memset(param->value, 0, param->bytesz);
		}
	}
		       ...
		       struct p4tc_act_param *param)
{
	...
	if (nla_put(skb, P4TC_ACT_PARAMS_VALUE_RAW, param->bytesz,
		    param->value))
		goto out_nlmsg_trim;
	nla_nest_end(skb, nla_value);

	if (param->mask &&
	    nla_put(skb, P4TC_ACT_PARAMS_MASK, param->bytesz, param->mask))
		goto out_nlmsg_trim;
		     ...
		     struct netlink_ext_ack *extack)
{
	...
	param->type = type;
	param->bitend = bitend;
	param->bytesz = BITS_TO_BYTES(bitend + 1);
