...
static inline u32 p4tc_fls(uint8_t *ptr, size_t len)
{
	int i;
...
static inline u16 find_lpm_mask(struct p4tc_table *table, u8 *ptr)
{
	u32 ret;
					     ...
					     struct p4tc_table_entry_mask *mask)
{
	...
	for (pos = 0; pos < nmasks; pos++) {
		u16 mask_value = find_lpm_mask(table, mask->fa_value);

		if (table->tbl_masks_array[pos]) {
			u16 array_mask_value;
