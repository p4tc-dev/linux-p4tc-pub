...
BTF_ID(struct, p4tc_ext_bpf_val)
...
__bpf_kfunc static struct p4tc_ext_bpf_val *
bpf_p4tc_extern_md_read(struct sk_buff *skb,
			...
			const u32 params__sz)
{
	...
	return p4tc_datapath_extern_md_read(net, params, params__sz);
}

__bpf_kfunc static int
bpf_p4tc_extern_md_write(struct sk_buff *skb,
			 struct p4tc_ext_bpf_params *params,
			 const u32 params__sz,
			 struct p4tc_ext_bpf_val *val, const u32 val__sz)
{
	struct net *net;

	net = skb->dev ? dev_net(skb->dev) : sock_net(skb->sk);

	return p4tc_datapath_extern_md_write(net, params, params__sz, val,
					     val__sz);
}

__bpf_kfunc static struct p4tc_ext_bpf_val *
xdp_p4tc_extern_md_read(struct xdp_buff *ctx,
			struct p4tc_ext_bpf_params *params,
			const u32 params__sz)
{
	struct net *net;
...
__bpf_kfunc static int
xdp_p4tc_extern_md_write(struct xdp_buff *ctx,
			 struct p4tc_ext_bpf_params *params,
			 const u32 params__sz,
			 struct p4tc_ext_bpf_val *val, const u32 val__sz)
{
...
BTF_KFUNCS_START(p4tc_kfunc_check_ext_set_skb)
BTF_ID_FLAGS(func, bpf_p4tc_extern_md_write);
BTF_ID_FLAGS(func, bpf_p4tc_extern_md_read, KF_RET_NULL);
BTF_KFUNCS_END(p4tc_kfunc_check_ext_set_skb)
...
BTF_KFUNCS_START(p4tc_kfunc_check_ext_set_xdp)
BTF_ID_FLAGS(func, xdp_p4tc_extern_md_write);
BTF_ID_FLAGS(func, xdp_p4tc_extern_md_read, KF_RET_NULL);
BTF_KFUNCS_END(p4tc_kfunc_check_ext_set_xdp)
