...
static const struct nla_policy tc_hdrfield_policy[P4TC_HDRFIELD_MAX + 1] = {
	[P4TC_HDRFIELD_DATA] = { .len = sizeof(struct p4tc_header_field_ty) },
	[P4TC_HDRFIELD_NAME] = { .type = NLA_STRING, .len = HDRFIELDNAMSIZ },
	[P4TC_HDRFIELD_PARSER_NAME] = { .type = NLA_STRING, .len = PARSERNAMSIZ },
};
		    ...
		    struct netlink_ext_ack *extack)
{
	...
	ret = nla_parse_nested_deprecated(tb, P4TC_HDRFIELD_MAX,
					  nla, tc_hdrfield_policy, extack);
	if (ret < 0)
		return ERR_PTR(ret);
