...
static const struct nla_policy p4tc_tclass_policy[P4TC_TCLASS_MAX + 1] = {
	[P4TC_TCLASS_NAME] = { .type = P4T_STRING, .len = TCLASSNAMSIZ },
	[P4TC_TCLASS_INFO] = { .len = sizeof(struct p4tc_table_class_parm) },
	[P4TC_TCLASS_PREACTIONS] = { .type = P4T_NESTED },
	[P4TC_TCLASS_KEYS] = { .type = P4T_NESTED },
	[P4TC_TCLASS_POSTACTIONS] = { .type = P4T_NESTED },
};

static const struct nla_policy p4tc_tclass_key_policy[P4TC_MAXPARSE_KEYS + 1] = {
	[P4TC_KEY_ID] = { .type = P4T_U32 },
	[P4TC_KEY_ACT] = { .type = P4T_NESTED },
};
...
{
	int ret = 0;
	struct nlattr *nest_keys = nla_nest_start_noflag(skb, key_num);
	struct nlattr *nest_action;

	if (nla_put_u32(skb, P4TC_KEY_ID, key->key_id))
		return -1;

	nest_action = nla_nest_start_noflag(skb, P4TC_KEY_ACT);
				  ...
				  struct p4tc_table_class *tclass)
{
	...
	nest_key = nla_nest_start_noflag(skb, P4TC_TCLASS_KEYS);
	...
	if (tclass->tbc_preacts) {
		preacts = nla_nest_start_noflag(skb, P4TC_TCLASS_PREACTIONS);
		if (tcf_action_dump(skb, tclass->tbc_preacts, 0, 0, false) < 0)
			goto out_nlmsg_trim;
		nla_nest_end(skb, preacts);
	}

	postacts = nla_nest_start_noflag(skb, P4TC_TCLASS_POSTACTIONS);
		    ...
		    struct netlink_ext_ack *extack)
{
	...
	if (tb_args) {
		...
		ret = nla_parse_nested_deprecated(tb_local, P4TC_TKEY_MAX,
						  nla, p4tc_tclass_key_policy,
						  extack);
		if (ret < 0)
			goto out;
		       ...
		       struct netlink_ext_ack *extack)
{
	...
	ret = nla_parse_nested_deprecated(tb_key, P4TC_TKEY_MAX, nla,
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	ret = nla_parse_nested_deprecated(tb, P4TC_MAXPARSE_KEYS, nla,
					  NULL, extack);
		  ...
		  struct netlink_ext_ack *extack)
{
	...
	ret = nla_parse_nested_deprecated(tb, P4TC_TCLASS_MAX, nla,
